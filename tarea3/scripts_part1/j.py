...
if __name__=='__main__':
    ...
    save_keras_model(model, path+name+'-LSTM')
    ...
    arch = open(path+name+'-LSTM.txt','w')
    ...
    model.fit(trainX, trainY, nb_epoch=100, batch_size=1, verbose=0)
    elapsed_time = time.time() - start_time
    save_keras_model(model, path+name+'-GRU')
    ...
    model.fit(trainX, trainY, nb_epoch=100, batch_size=1, verbose=0)
    elapsed_time = time.time() - start_time
    save_keras_model(model, path+name+'-RNN')
