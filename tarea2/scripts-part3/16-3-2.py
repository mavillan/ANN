...
if __name__=='__main__':
    ...
    cp = 2
    model = Sequential()
    model.add(Convolution2D(fl, cf, cf, border_mode='same', activation='relu',input_shape=(n_channels, n_rows, n_cols)))
    model.add(MaxPooling2D(pool_size=(cp, cp)))
    model.add(Convolution2D(fl, cf, cf, border_mode='same', activation='relu'))
    model.add(MaxPooling2D(pool_size=(cp, cp)))
    model.add(Flatten())
